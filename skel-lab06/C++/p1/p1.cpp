...
Nim::apply_move(const Move &move)
{
     ...
     */
     ...
     if (this->heaps[move.heap] < move.amount)
        return false;
...
minimax(Nim init, int player, int depth)
{
    ...
    if (init.ended() || depth == 0) {
        int heap = 0;
        while (init.heaps[heap] < 3 && init.heaps[heap] != 0) heap++;
        return std::make_pair(init.eval(player), Move(1, heap));
    }
     ...
     */
     ...
     for (auto move : moves) {
         ...
         std::pair<int, Move>sc = minimax(init, (-1) * player, depth - 1);
         sc.first = 0 - sc.first;
         if (sc.first >= max) {
             max = sc.first;
             m = move;
         }
         init.undo_move(Move(move.amount, move.heap));
     }

    return std::make_pair(max, m);
}
